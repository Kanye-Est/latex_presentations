...
\begin{frame}{3.1 解决方案一：强制节点排序 (视觉化图解)}
    \textbf{核心思路：既然每次训练出来的神经元位置都是乱的，那我们在展平前，干脆强行用数学规则把它们“按资排辈”。}
    
    \begin{columns}
        \begin{column}{0.45\textwidth}
            \textbf{如图 4 所示的 Canonical Form 转换：}
            \begin{enumerate}
                \item \textbf{制定排序规则：} 计算每个神经元所有输入权重的和（或绝对值之和）。如图(a)中，$n_1^1$ 的权和为 $4.0-0.5+2.0=5.5$，$n_2^1$ 为 $3.0+1.0=4.0$，$n_3^1$ 为 $2.0-1.0=1.0$。
                \item \textbf{强制重排：} 按照身价从大到小，对神经元位置进行行交换。图(b)将原 $n_2^1$ 和 $n_1^1$ 进行了互换。
                \item \textbf{同步补偿：} 神经元位置变了，为防止网络逻辑错乱，下一层接收数据的权重矩阵也必须做\textbf{完全同步的列交换}（如右图的 $0.3$ 和 $2.4$ 连线也跟着互换了）！
            \end{enumerate}
        \end{column}
        \begin{column}{0.55\textwidth}
            \begin{center}
    ...
    \begin{columns}
        \begin{column}{0.45\textwidth}
            ...
            \begin{enumerate}
                ...
                \item \textbf{Summation ($\Sigma$)：} \textcolor{myred}{\textbf{核心魔法！}} 对同层所有节点特征进行\textbf{求和}得到 $L_t$。由于 $A+B=B+A$，强行抹除了节点的排列顺序噪音。
