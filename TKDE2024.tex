...
\begin{frame}{3.3 方案 B：针对不可微模型的通用攻击 (Algorithm 3)}
    \begin{columns}
        \begin{column}{0.45\textwidth}
            \textbf{Generic Distribution Inference Attack：}
            \begin{itemize}
                \item \textbf{挑战：} 树模型等不可微模型无法直接求导。
                \item \textbf{搜索策略 (Search)：} 采用启发式搜索。
                \item \textbf{自适应调整：}
                \begin{itemize}
                    \item \textbf{行 9-11：} 如果多次合成失败，则放松阈值 $\tau_{min}$。
                    \item \textbf{行 29-30：} 如果拒绝率过高，则动态缩小搜索空间维度 $d$。
                \end{itemize}
                \item \textbf{目标：} 在离散/不可微的参数空间中寻找最优分布近似。
            \end{itemize}
        \end{column}
        \begin{column}{0.55\textwidth}
            \begin{center}
                \includegraphics[width=0.9\textwidth]{TKDE2024pic/alg3_generic.png}
            \end{center}
        \end{column}
    \end{columns}
\end{frame}

\begin{frame}{3.4 PGDA 数学本质：基于梯度匹配的优化}
    攻击者求解以下优化问题来逼近真实分布：
    \begin{equation}
    ...
    \textbf{关键洞察：} 该过程不仅能还原标签比例，还能通过梯度信息反推出特征维度的统计量。
\end{frame}
...
\begin{frame}{4.1 击穿最强防御：为什么 DP 失效了？}
    \begin{columns}
        ...
        \begin{column}{0.55\textwidth}
            \begin{center}
                \includegraphics[width=\textwidth]{TKDE2024pic/fig:result.png}
            \end{center}
        \end{column}
    \end{columns}
\end{frame}
