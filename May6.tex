...
\begin{frame}{3.4 DeepSets 函数映射解析}
    公式 $f(X) = \rho( \sum_{x \in X} \phi(x) )$ 中的三个核心算子映射及其职责：
    \begin{itemize}
        \setlength{\itemsep}{10pt}
        \item \textbf{$\phi$ 映射 (实例级特征提取网络)：} 
        \begin{itemize}
            \item \textbf{机制：} 独立且非线性地映射集合 $X$ 中的每一个神经元特征 $x$。
            \item \textbf{作用：} 将低级的原始权重参数转换为高维的隐空间表示。同层各个神经元共享 $\phi$ 网络的参数。
        \end{itemize}
        \item \textbf{$\sum$ 算子 (对称聚合操作)：} 
        \begin{itemize}
            \item \textbf{机制：} 对所有神经元经 $\phi$ 提取的隐层表示进行求和。
            \item \textbf{作用：} 利用加法交换律，该操作在数学上\textbf{严格保证了排列不变性 (Permutation Invariance)}，原生抹除了神经元的排列位置噪音。
        \end{itemize}
        \item \textbf{$\rho$ 映射 (全局读出网络)：}
        \begin{itemize}
            \item \textbf{机制：} 接收经对称聚合后得到的层级全局表示 (Global Representation)。
            \item \textbf{作用：} 将脱离了排列约束的纯净层级特征，映射至最终的属性判别空间，输出模型具备特定宏观属性的概率。
        \end{itemize}
    \end{itemize}
\end{frame}
