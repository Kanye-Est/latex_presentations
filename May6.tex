...
\begin{frame}{3.1 解决方案一：强制节点排序 (视觉化图解)}
    \textbf{核心思路：在参数展平前，基于固定规则对神经元进行强制重排对齐。}
    ...
    \begin{columns}
        \begin{column}{0.45\textwidth}
            ...
            \begin{enumerate}
                \setlength{\itemsep}{8pt}
                \item \textbf{计算指标：} 按入边权重绝对值求和。图(a)中，$n_1^1$ 权和 $5.5 > n_2^1$ 的 $4.0$。
                \item \textbf{执行行交换：} 按指标从大到小重排。图(b)将原 $n_2^1$ 和 $n_1^1$ 上下互换。
                \item \textbf{列交换补偿：} 为保证输出不变，下一层权重必须\textbf{同步列交换}（注意右图 $0.3$ 和 $2.4$ 连线的交叉）。
            \end{enumerate}
        \end{column}
        \begin{column}{0.55\textwidth}
            \begin{center}
